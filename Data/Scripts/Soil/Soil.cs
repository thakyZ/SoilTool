...
*            Modified by HackerFinn.             *
*      Website: https://www.codeslasher.com/     *
* All credit for the original code goes to Digi. *
**************************************************/
using System;
...
namespace HackerFinn.Soil
{
  ...
  public class Soil : MySessionComponentBase
  {
    public override void LoadData()
    {
      Log.SetUp("Soil Tool", 396679430, "SoilTool");
    }

    public static Soil instance = null;
    ...
    public const int NUMBER_OF_MATERIALS = 7;
    public readonly string[] MATERIAL_NAMES = new string[NUMBER_OF_MATERIALS] { "Grass", "AlienSand", "AlienRockGrass", "MarsSoil", "MoonRocks", "TritonStone", "Snow" };
    public const int NUMBER_OF_ICE = 1;
    public readonly int[] ICE_INDEX_VALUES = new int[NUMBER_OF_ICE] { 6 };
    public static MyVoxelMaterialDefinition[] MATERIALS = new MyVoxelMaterialDefinition[NUMBER_OF_MATERIALS];
    public static int currentMaterialIndex = 0;
    public const string SOIL_TOOL = "SoilTool";
    public const string SOIL_WEAPON_ID = "WeaponSoilTool";
    public const string SOIL_AMMO_ID = "DirtBag";
    public const string ICE_AMMO_ID = "IceBag";
    public const string SOIL_GHOST_ID = "SoilToolGhost";
    private readonly MyObjectBuilder_AmmoMagazine SOIL_MAG = new MyObjectBuilder_AmmoMagazine() { SubtypeName = SOIL_AMMO_ID, ProjectilesCount = 1 };
    private readonly MyDefinitionId SOIL_MAG_DEFID = new MyDefinitionId(typeof(MyObjectBuilder_AmmoMagazine), SOIL_AMMO_ID);
    private readonly MyObjectBuilder_AmmoMagazine ICE_MAG = new MyObjectBuilder_AmmoMagazine() { SubtypeName = ICE_AMMO_ID, ProjectilesCount = 1 };
    private readonly MyDefinitionId ICE_MAG_DEFID = new MyDefinitionId(typeof(MyObjectBuilder_AmmoMagazine), ICE_AMMO_ID);

    private static readonly MyStringId MATERIAL_SQUARE = MyStringId.GetOrCompute("SoilTool_Square");
    private static readonly MyStringId MATERIAL_FADEOUTLINE = MyStringId.GetOrCompute("SoilTool_FadeOutLine");
    private static readonly MyStringId MATERIAL_FADEOUTPLANE = MyStringId.GetOrCompute("SoilTool_FadeOutPlane");

    public const float SOIL_PLACE_USE = 1f;
    public const float SOIL_PAINT_USE = 0.5f;
    ...
    public void Init()
    {
      ...
      {
        if (MATERIALS[i] == null && !MyDefinitionManager.Static.TryGetVoxelMaterialDefinition(MATERIAL_NAMES[i], out MATERIALS[i]))
        {
          throw new Exception("ERROR: Could not get the '" + MATERIAL_NAMES[i] + "' voxel material!");
        }
      }

      // make the soil tool not be able to shoot normally, to avoid needing to add ammo and the stupid hardcoded screen shake
      var gunDef = MyDefinitionManager.Static.GetWeaponDefinition(new MyDefinitionId(typeof(MyObjectBuilder_WeaponDefinition), SOIL_WEAPON_ID));
    ...
    {
      instance = null;
      MATERIALS[currentMaterialIndex] = null;
    ...
    private void VoxelAction(PacketType type, IMyVoxelBase voxels, IMyVoxelShape shape, float scale, byte materialIndex = 0, IMyCharacter character = null)
    {
      if (MyAPIGateway.Session.IsServer)
      {
        ...
        if (place || type == PacketType.PAINT_VOXEL)
        {
          ...
          if (!ICE_INDEX_VALUES.Contains(currentMaterialIndex))
          {
            if (inv.GetItemAmount(SOIL_MAG_DEFID) >= use)
            {
              if (place)
              ...
              inv.RemoveItemsOfType(use, SOIL_MAG, false);
            }
            else
          ...
          {
            if (inv.GetItemAmount(ICE_MAG_DEFID) >= use)
            {
              if (place)
                MyAPIGateway.Session.VoxelMaps.FillInShape(voxels, shape, materialIndex);
              else
              ...
              inv.RemoveItemsOfType(use, ICE_MAG, false);
            }
            else
              Log.Error($"Not enough ammo ({use}) for {type} on {character.DisplayName} ({character.EntityId})");
          }
    ...
    public void DrawTool(IMyAutomaticRifleGun gun)
    {
      ...
      if (!seenHelp)
        SetToolStatus($"Press {InputHandler.GetAssignedGameControlNames(MyControlsSpace.SECONDARY_TOOL_ACTION)} to see Soil Tools' advanced controls.", 1500, MyFontEnum.White);
    }
    ...
    public void HoldingTool(IMyCharacter character)
    {
      ...
      if (selectedVoxelMap == null)
      {
        if (trigger || paint)
        {
          SetToolStatus("Soil can only be placed on planets or asteroids!", 1500, MyFontEnum.Red);
        }
      }
      else
      {
    ...
    private bool ToolProcess(IMyVoxelBase voxels, Vector3D target, MatrixD view, bool primaryAction, bool paintAction)
    {
      ...
      if (inputReadable)
      {
        ...
        if (ctrl && shift)
        {
          var matScroll = Input.DeltaMouseScrollWheelValue();
        ...
        if ((rotateInput.X != 0 || rotateInput.Y != 0 || rotateInput.Z != 0))
        {
          ...
          SetAlignStatus($"Align: Custom - {Math.Round(MathHelper.ToDegrees((float)angles.X))}° / {Math.Round(MathHelper.ToDegrees((float)angles.Y))}° / {Math.Round(MathHelper.ToDegrees((float)angles.Z))}°", 500, FONTCOLOR_INFO);
        }
      ...
      if ((primaryAction && !removeMode) || paintAction)
      {
        ...
        if (ICE_INDEX_VALUES.Contains(currentMaterialIndex) == false && inv.GetItemAmount(SOIL_MAG_DEFID) < GetAmmoUsage(type, placeScale))
        {
          PlaySound("HudUnable");
          SetToolStatus($"You need Dirt Bag to use this tool!", 1500, MyFontEnum.Red);
          return false;
        }
        else if (ICE_INDEX_VALUES.Contains(currentMaterialIndex) == true && inv.GetItemAmount(ICE_MAG_DEFID) < GetAmmoUsage(type, placeScale))
        {
          PlaySound("HudUnable");
          SetToolStatus($"You need Ice Bag to use this tool!", 1500, MyFontEnum.Red);
          return false;
        }
      }
      ...
      if (paintAction)
      {
        VoxelAction(PacketType.PAINT_VOXEL, voxels, placeShape, placeScale, MATERIALS[currentMaterialIndex].Index, Session.Player.Character);
      ...
      else if (primaryAction)
      {
        ...
        else
        {
          ...
          cooldown = cooldownTicks;
          VoxelAction(PacketType.PLACE_VOXEL, voxels, placeShape, placeScale, MATERIALS[currentMaterialIndex].Index, Session.Player.Character);
    ...
    private void ChangeMaterial(int index)
    {
      ...
      var newIndex = currentMaterialIndex + index;
      ...
      SetToolStatus("Current material: " + MATERIAL_NAMES[currentMaterialIndex], 1500, MyFontEnum.White);
    }

    private static MyFixedPoint GetAmmoUsage(PacketType type, float scale)
    {
      if (type == PacketType.PLACE_VOXEL)
        return (MyFixedPoint)(SOIL_PLACE_USE * scale);

      if (type == PacketType.PAINT_VOXEL)
        return (MyFixedPoint)(SOIL_PAINT_USE * scale);
    ...
    {
      if (msg.StartsWith("/soil", StringComparison.InvariantCultureIgnoreCase))
      {
        send = false;

        msg = msg.Substring("/soil".Length).Trim();
        ...
        Utilities.ShowMessage("/soil help ", "key combination information");
      }
    }
    ...
    private void ShowHelp()
    {
      ...
      //var inputNextMaterial = InputHandler.GetAssignedGameControlNames(MyControlsSpace.NEXT_BLOCK_STAGE, true);
      //var inputPrevMaterial = InputHandler.GetAssignedGameControlNames(MyControlsSpace.PREV_BLOCK_STAGE, true);
      ...
      str.AppendLine("The soil tool is a hand-held tool that allows placement of soil\n  on to asteroids or planets.");
      ...
      str.AppendLine();
      str.Append(inputFire).Append(" = place soil.").AppendLine();
      str.AppendLine();
      str.Append("Ctrl+Shift+MouseScroll/Plus/Minus = change soil material.").AppendLine();
      str.AppendLine();
      str.Append(inputPaint).Append(" = replace terrain with soil.").AppendLine();
      str.AppendLine();
      ...
      Utilities.ShowMissionScreen("Soil Tool Help", string.Empty, string.Empty, str.ToString(), null, "Close");
    }
  }
}